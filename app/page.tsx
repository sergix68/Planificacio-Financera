...
export default function Home() {
  ...
  const importPlan = (file: File) => { file.text().then((raw) => { try { const parsed = JSON.parse(raw) as { inputs?: Partial<Inputs>; scenario?: Scenario }; if (parsed.inputs) setInputs((current) => ({ ...current, ...parsed.inputs })); if (parsed.scenario && parsed.scenario in scenarioLabels) setScenario(parsed.scenario); setSaved(true); } catch { setSaved(false); } }); };
